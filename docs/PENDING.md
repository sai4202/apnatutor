...
**Last reviewed:** 2026-09-04 (M5 closed)
...
M5 is complete. Everything below belongs to **M6 — Polish & launch**, which is deferred at your request. Nothing in it is blocked.

1. `M6-04` / `M6-05` — seed and demo data, then end-to-end tests. The cheapest first step, and the one that makes everything after it easier to check.
2. `M6-07` — deployment, where **T13**, **T20** and **T21** all come due together: two are the same single-instance assumption, the third only becomes fixable once a proxy exists.
3. `M6-09` / `M6-10` — legal pages and production providers. `D5` (SMS provider) and `D6` (GST invoicing) are the open decisions that gate these, and both have been researched but not decided.
4. **`T11` — delete `lib/exampleTutors.ts`.** Not a milestone task, and the one piece of debt that becomes a real problem rather than a slow one if it is forgotten.
